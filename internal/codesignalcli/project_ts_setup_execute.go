...
// directories.
var ErrSetupExecutionHazardousWorkingDirectory = errors.New("setup execution: working directory carries a package-manager configuration hazard")
...
// timed-out run belongs to RunConfirmedSetup (below) -- ExecuteSetup only
...
// confirmation from the user before this call; ExecuteSetup itself neither
...
func ExecuteSetup(ctx context.Context, preview SetupPreview, confirmed bool) (SetupExecutionResult, error) {
	...
	// would incorrectly trip RunConfirmedSetup's failure-handling path).
...
// may proceed -- RunConfirmedSetupAndRecheckReadiness's post-install
