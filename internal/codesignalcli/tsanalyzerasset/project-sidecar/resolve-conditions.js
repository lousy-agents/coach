...
export function resolveConditions(value) {
    if (typeof value === "string")
        return stripLeadingDot(value);
    if (!isRecord(value))
        return undefined;
    for (const condition of CONDITION_PRIORITY) {
        const nested = value[condition];
        if (typeof nested === "string")
            return stripLeadingDot(nested);
...
function nestedRecord(nested) {
    if (!isRecord(nested))
        return undefined;
    return resolveConditions(nested);
}
export function stripLeadingDot(p) {
    return p.replace(/^\.\//, "");
}
//# sourceMappingURL=resolve-conditions.js.map