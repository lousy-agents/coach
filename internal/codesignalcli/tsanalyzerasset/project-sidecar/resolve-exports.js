 ...
 */
export function resolvePackageExports(exp, main, subpath) {
...
function mainFallback(exp, main, subpath) {
    if (exp !== undefined || main === undefined || subpath !== ".")
        return undefined;
    return stripLeadingDot(main);
}
function resolveExportsObject(exp, subpath) {
    const direct = exp[subpath];
    if (direct !== undefined)
        return resolveConditions(direct);
    return resolveWildcardExport(exp, subpath);
}
function resolveWildcardExport(exp, subpath) {
    for (const [pattern, value] of Object.entries(exp)) {
        ...
        if (resolved !== undefined)
            return resolved.replace("*", match);
    }
    return undefined;
}
function tryWildcard(pattern, subpath) {
    if (!pattern.includes("*"))
        return undefined;
