...
- **`RequestRecord`** (`fixture.go`): the minimum shared shape a fixture-driven fake service must record for every request it handles — `SchemaVersion int`, `Scenario string`, `Method string`, `Path string`, `AuthMode AuthMode` (JSON: `schema_version`, `scenario`, `method`, `path`, `auth_mode`). Per the epic, "Request recording is part of the contract, not debug logging." Build one via `NewRequestRecord(scenario, method, path string, mode AuthMode) RequestRecord`, which stamps `SchemaVersion` with the current `FixtureSchemaVersion` so callers can't accidentally omit or mismatch it.
