...
- The role check uses `GET /repos/{owner}/{repo}/collaborators/{username}/permission`, which returns the principal's *effective* permission including team- and org-derived access. The local Path C manifest (`deploy/compose/platform/github-app.manifest.json`) requests **Administration (read)** for this endpoint, in addition to Contents and Metadata (read). That grant is wider than a Contents-only app; it is the implemented groundwork manifest, not a verified least-privilege proof against GitHub's current permission mapping. If a real installation shows Administration is unnecessary, or if it demands more than this manifest, revisit before copying it into the webhook-platform broker. The fallback (org membership + team enumeration) needs org `Members` permission — a materially different manifest that would need a fresh decision.
