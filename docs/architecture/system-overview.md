...
Contrast with the flow above: the worker path consumes an immutable snapshot through a queue and produces a governed evidence artifact as part of a larger workflow; the CLI path is a developer running one command against their own worktree, printing a report to their own terminal. Both share the same deterministic `pkg/semantics`/`pkg/codesignal` core, but the CLI path stays local and advisory — it does not run customer tests or builds, does not prove a defect, and does not publish GitHub feedback or block CI on its own. TypeScript project analysis does spawn the resolved host Node runtime and TypeScript compiler under Analysis-phase confinement.

TypeScript project analysis distinguishes three phases that must not be collapsed. **Preparation** is an explicitly approved phase that may use the network, run one displayed package-manager or mise command, and mutate only the selected manifest/lockfile or mise configuration; `--check-project` is the read-only fit result that precedes it. **Analysis** is an offline, non-mutating phase confined to immutable Git source plus an explicitly resolved host compiler. **Policy authoring** is an interactive, create-only preparation action (`--suggest-project-config --project-language typescript`) whose output must be reviewed and committed before analysis. The superseded #208 contract that required a vendored `js/semantics/bin/coach-ts-project-sidecar` lookup inside the analyzed repository is retired. Read-only mise probes (`mise config get`, `mise where`) run with a neutral working directory and a minimal environment (`PATH`, `HOME`, and mise data/config directories) so the analyzed repository's `mise.toml` exec templates never run during readiness or analysis.

An explicit `--project-config` opts into one narrow exception to "no cross-file analysis": layer-boundary checks for Go and TypeScript/TSX (`architecture.layer_violation`, `pkg/codesignal/rule_layer_violation.go`'s `EvaluateGoLayerViolations` and `EvaluateTypeScriptLayerViolations`, wired through `internal/codesignalcli/project_go_backend.go` and `internal/codesignalcli/project_ts_backend.go` respectively). The TypeScript backend always emits `possible_call_reachability` `ProjectFacts` (head revision only; `internal/codesignalcli/project_ts_backend.go`'s `evaluateRevision`, via `projectmodel.BuildTypeScriptReachabilityFromModel` and `codesignal.ReachabilityProjectFacts`), unlike the Go backend, whose `project_go_backend.go` never populates `ProjectBackendResult.Facts`; with `required_layer` configured it additionally emits `architecture.layer_bypass` ProjectChanges (see below). Both read the same configured directory-prefix layers and forbidden import pairs and report which internal import edges cross a forbidden layer boundary, but they build their `pkg/projectmodel` import graph differently: the Go backend builds it purely from `git ls-tree`/`git show` at the selected revision(s), with no subprocess involved, while the TypeScript backend instead builds it from Coach's embedded analyzer plus the host-resolved TypeScript compiler (resolution is per selected policy root: nearest `package.json` at or above that root, bounded by the repository; every selected root must agree on the probed installed version or the run fails with `typescript_version_conflict` naming each root's finding; the worktree top is the walk ceiling, not the sole origin). The three origins — project manifest context, project mise, global mise — are evaluated in that precedence, each yielding one class: the five candidate classes `eligible`, `unsupported`, `native-invalid`, `absent` and `unreadable`, or `unconfigured` for an origin that names no compiler at all. The project origin's candidate is the compiler *installed* at each selected root's manifest context, classed by its probed version alone; a manifest declaration never decides that class, and a declaration with nothing installed beside it leaves the origin absent, whatever it declares. The first eligible candidate wins; a rejected candidate never stops a lower-precedence origin, so a root with no manifest context, an unreadable manifest, an uninstalled pin, or a compiler missing its native package all leave mise free to supply the compiler. Only a manifest naming exactly one exact in-set version admits the project package to *setup choices*, mirroring the rule the mise scopes follow, since the frozen install command installs what is declared and nothing else; a missing, non-exact, or out-of-set declaration excludes it from those choices only: the compiler installed beside it is still that origin's candidate, so a caret-range project with a supported install passes from its own origin, and an out-of-set install is reported as the unsupported compiler that is present rather than as none found, while `typescript_compiler_missing` carries `found_version` only in the native-invalid class. The declaration governs warnings: when a non-project origin wins, any selected root declaring a different version raises `compiler_declaration_mismatch`; when the project origin wins, only a stale *exact* pin does, and range satisfaction is never evaluated. An ambiguity *inside* one origin is the exception: two declarations in one manifest, or two exact pins in one mise `[tools]` table, require an explicit selection and stop resolution there. Per-root disagreement is a conflict regardless of any mise candidate. Readiness and the scan are two projections of that one aggregation over the same inputs, so for a given worktree state, selected roots and host tooling a `--check-project` compiler pass means the scan resolves that same compiler instead of refusing — eligibility proves the package is present and carries a version-equal native package, not that the resolved runtime will load it, and a probe that times out between the two invocations can still change the answer. A scan that refuses names the same gap code on one stderr line, with each root's finding for a conflict and for a mismatch a selected root's own compiler caused. The analyzer is handed that same immutable Git snapshot's file contents over stdin — Analysis-phase confinement: no network (analyzer child does not inherit HTTP(S)_PROXY and a recording proxy is not dialed), source/`tsconfig`/package metadata/aliases/customer imports exclusively from that snapshot (`readFile` misses return null rather than host content), and ambient Node loader/proxy/package-manager configuration stripped from the analyzer environment. Directory-listing miss fall-through is closed. Explicit native spawn-path is closed. A TypeScript project scan is an offline, non-mutating analysis of the selected Git snapshot. During the scan Coach may read committed project source from that snapshot; the host compiler and runtime files it already resolved before the scan started; Coach's own analyzer files; and ordinary system libraries. It does not use the network during the scan, and it does not treat host files as a substitute for missing project source. Linux CI acceptance records that this allowed set holds on Linux. The same rule is the intended behavior on macOS and Windows and is inferred from that Linux evidence, not separately proven there. This is not a claim that the operating system sandboxes the process, and it is not a claim that the target TypeScript customer journey ships. The internal subprocess boundary is not a customer-vendored binary. This stays advisory (never blocks anything or changes the CLI's exit-code contract on its own), coverage-honest (an edge that is unresolved or does not map to a configured layer contributes zero findings rather than a guess; an incomplete project model instead retains its findings with lifecycle `unknown` plus a `project_lifecycle_indeterminate` diagnostic, never a silent claim of introduced/existing/resolved), approximate (directory-prefix layer membership over import facts, not a full type or build-graph analysis), and local-only (no GitHub, queue, or model-gateway access, same as the rest of this CLI path). `go` and `typescript` are the only implemented `--project-language` values today; any other value is rejected outright as a usage error. The absence of an `architecture.layer_violation` finding is never proof a codebase follows a particular architecture: it can mean the edge does not violate the configured policy, the edge is not covered by any declared layer, or project coverage was incomplete for that revision. Omitting `--project-config` leaves the CLI on the unchanged schema-1, file-local-only path described above.

The prepared analysis runtime records executable path, spawn arguments, exact version, kind, and origin. Spawn uses that executable path and those arguments. Kind is the selected runtime family; origin records how it was discovered.

Fit checks may read the worktree to report gaps. Analysis reads only the selected committed snapshot for source, configuration, and package metadata. A dirty worktree may change what setup is offered; it shall not change what the report claims about the revision.

Read-only mise probes run with a working directory that is not the analyzed repository and with an environment limited to PATH, HOME, and mise data/config directories, so repository exec templates never run during readiness or analysis.

The report records the compiler's exact version and origin. It does not record a package digest; that attestation is omitted in this phase.

The future CodeSignal worker shall reuse this same analysis-phase confinement contract — snapshot-only customer inputs, the same host-read allowlist, no network during analysis, and the same prepared-runtime identity. The CLI does not substitute for the worker; it previews the same confined analyzer.

`--project-config`'s optional `required_layer` opts into a second, narrower cross-file check: `architecture.layer_bypass` (`pkg/codesignal/rule_layer_bypass.go`'s `EvaluateGoLayerBypass` and `EvaluateTypeScriptLayerBypass`, wired through `internal/codesignalcli/project_go_backend.go` and `internal/codesignalcli/project_ts_backend.go` respectively). Where `architecture.layer_violation` flags a forbidden import edge, `architecture.layer_bypass` flags a statically resolved call path from a handler-shaped source to a pinned sink (e.g. a database call) that structurally never passes through the configured required layer — a route that can silently skip that layer's invariants (validation, authorization, caching, ...) even when a separate, compliant route also exists. The underlying search (`pkg/projectmodel.BuildGoLayerBypass`) reports its own `Coverage`, separate from and in addition to the project model's own coverage, because the two measure different things: how completely the source/sink call-graph search itself completed, versus how completely the import graph was built. `coach codesignal`'s incomplete-coverage policy for this search is deliberately **emit, not suppress**: a witness a search already found is a real, statically resolved path regardless of what else the same run failed to finish searching, so it is never hard-dropped just because coverage elsewhere in that same run was incomplete. Instead, three things happen together, matching `EvaluateGoLayerBypass`'s own documented per-witness contract: (1) any `architecture.layer_bypass` witness found within an incomplete search is still emitted as a `ProjectChange`; (2) a `project_layer_bypass_coverage_incomplete` diagnostic is attached to the report, so the incompleteness stays visible rather than silently dropped; and (3) the search's own `Coverage.Complete = false` is folded into the project's overall `Coverage` (`internal/codesignalcli/project_go_backend.go`'s `combineProjectCoverage`), which degrades **every project change in the report** — `architecture.layer_bypass` and `architecture.layer_violation` alike, on both the head and the base revision — to lifecycle `unknown` (`projectLifecycleState` in `pkg/codesignal/codesignal.go`) instead of a definitive `resolved`/`introduced`/`existing`/`baseline` claim, because `projectLifecycleState` tracks one report-wide indeterminate flag rather than a per-revision one: an incomplete search on the *base* side alone degrades the *head* side's changes too, even when the head-side search itself completed cleanly. This is because the absence of a witness for some other source/sink pair in the same truncated run is not a sound "no bypass" claim, and a lifecycle transition computed against that gap (e.g. "resolved") could just as easily be an artifact of the truncation as a real fix. This is the same trade-off `architecture.layer_violation` already makes for an incomplete project model, made CLI-visible and explicit here rather than left implicit in the library's own doc comment. The TypeScript backend folds this Coverage differently on purpose: `project_ts_backend.go`'s `tsBypassCoverageForFold` strips `BuildTypeScriptLayerBypassFromModel`'s own reachability-gap term (`tsReachabilityHasGap`) back out of the bypass search's `Coverage.Complete` before `combineProjectCoverage` folds it in, so a routine per-hop reachability gap alone never degrades an otherwise-complete layer finding to lifecycle `unknown` — only a search that genuinely hit its budget or ran against an ambiguous required layer still does (AC-3/AC-EVD-5). In `--base` (diff) mode, a `project_layer_bypass_coverage_incomplete` diagnostic from the base-side search is distinguished from a head-side one by a `base_` prefix on its `Kind` (mirroring the existing `syntax_errors`/`base_syntax_errors` convention), so two independently incomplete searches never collapse into indistinguishable duplicate diagnostics. The search itself has no production node-count cap: its only bound is `Budgets.WallTime` (`goProjectBudgets`, `internal/codesignalcli/project_go_backend.go`), the same wall-clock budget every other phase of this backend already shares.

The same coverage-honesty principle extends to the base schema-1 path's own zero-active-finding verdict: `internal/codesignalcli/render.go`'s renderer never prints the unqualified "no findings" sentence when `Diagnostics` is non-empty or project coverage (on either side of a diff) did not complete — it qualifies the verdict instead, naming what was skipped or left indeterminate, so an incomplete analysis cannot read as a clean one.
