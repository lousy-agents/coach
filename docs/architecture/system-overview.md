...
An explicit `--project-config` opts into one narrow exception to "no cross-file analysis": a Go layer-boundary check (`architecture.layer_violation`, `pkg/codesignal/rule_layer_violation.go`, wired through `internal/codesignalcli/project_go_backend.go`). It reads the configured directory-prefix layers and forbidden import pairs, builds a `pkg/projectmodel` Go import graph purely from `git ls-tree`/`git show` at the selected revision(s), and reports which internal import edges cross a forbidden layer boundary. This stays advisory (never blocks anything or changes the CLI's exit-code contract on its own), coverage-honest (an edge that is unresolved or does not map to a configured layer contributes zero findings rather than a guess; an incomplete project model instead retains its findings with lifecycle `unknown` plus a `project_lifecycle_indeterminate` diagnostic, never a silent claim of introduced/existing/resolved), approximate (directory-prefix layer membership over import facts, not a full type or build-graph analysis), and local-only (no GitHub, queue, or model-gateway access, same as the rest of this CLI path). Go is the only implemented `--project-language` today; `typescript` is accepted by the flag parser but has no registered backend yet (exit `3`, `project_backend_unavailable`); any other value is rejected outright as a usage error. The absence of an `architecture.layer_violation` finding is never proof a codebase follows a particular architecture: it can mean the edge does not violate the configured policy, the edge is not covered by any declared layer, or project coverage was incomplete for that revision. Omitting `--project-config` leaves the CLI on the unchanged schema-1, file-local-only path described above.

`pkg/projectmodel` separately builds bounded, deterministic Go possible-call-reachability facts (`BuildGoCallGraph`, `BuildGoReachability`): a pinned static call graph over direct, statically resolved calls — interface dispatch, unresolved function values, reflection, and framework-registration handoffs each contribute a coverage diagnostic rather than a guessed edge — plus a deterministic shortest-path search from a fixed HTTP-handler-shaped source registry to a fixed sink-name registry, bounded by `GoBudgets`' `WallTime`/`MaxGraphNodes`/`MaxGraphEdges` (enforced here, unlike `BuildGoModel`, which enforces only the input-file/byte budgets) plus `ReachabilityOptions.MaxSearchNodes` for the search itself (`DiagCallGraphBudgetExceeded`/`DiagReachabilityBudgetExceeded` on exhaustion). Call-graph edges are direct, statically resolved calls only, not a full points-to analysis; it is facts-only, so a missing `ReachabilityFact` for a source/sink pair means only "no path found within the coverage this run achieved", never a "safe" or "verified absent" claim. It remains library-only today: `internal/codesignalcli`'s Go project backend (`project_go_backend.go`) calls only `codesignal.EvaluateGoLayerViolations`, and no `coach codesignal` report surfaces call-graph or reachability facts yet.
