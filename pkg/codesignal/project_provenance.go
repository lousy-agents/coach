...
// snapshot metadata. Kind and origin come from the analyzed revision;
// Version is the host-probed executable version and is omitted when
// unverifiable (SA-280-033).
...
func normalizeProvenance(p *ProjectProvenance) *ProjectProvenance {
	if p == nil {
		return nil
	}
	cp := *p
	cp.SelectedRoots = nonNilSlice(cp.SelectedRoots)
	return &cp
}
