...
// computeFeatures walks root exactly once, producing both the structural
...
func computeFeatures(root *tree_sitter.Node, source []byte) (StructuralMetrics, []Finding) {
	c := &featureCollector{}
...
// contains a pointer_type: either result itself is a pointer_type (single
// unnamed return value), or result is a parameter_list whose
// parameter_declaration children include one with a pointer_type "type"
// field (multiple and/or named return values).
func resultHasPointerType(result *tree_sitter.Node) bool {
	...
	for i := uint(0); i < count; i++ {
		...
		if typ != nil && typ.Kind() == "pointer_type" {
			return true
		}
	}
	return false
