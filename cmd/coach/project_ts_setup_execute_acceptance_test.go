...
var _ = Describe("codesignalcli.ExecuteSetup", func() {
	When("preview.WorkingDirectory itself carries a package-manager configuration hazard (a monorepo package directory, not the hazard-checked worktree root)", func() {
		It("refuses without starting any subprocess, rather than executing against an unverified .npmrc registry redirect (fail-closed)", func() {
			workDir := newSetupExecutionWorkDir()
			Expect(os.WriteFile(filepath.Join(workDir, ".npmrc"), []byte("registry=https://mirror.example.invalid/npm/\n"), 0o644)).To(Succeed())
...
// requireRealPackageManager fails the current spec when name is not installed
// on this machine. The sentinel-script proofs below are the only evidence
// that each frozen adapter row actually suppresses a dependency's lifecycle
// scripts, and a stand-in binary cannot produce it -- so a missing manager
// makes the proof unavailable, which is a broken toolchain to repair rather
// than a result to report as green. mise.toml pins pnpm and bun alongside go
// and node for exactly this reason, and CI installs them for the verify job.
func requireRealPackageManager(name string) {
	_, err := exec.LookPath(name)
	Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("%s is not on PATH, so the real-execution sentinel-script proof for %s cannot run; install it (mise.toml pins it) rather than letting this proof vanish", name, name))
}
...
func writeSentinelRootPackageJSON(repoDir string, allowPnpmBuild, trustBunDependency bool) {
	...
	if allowPnpmBuild {
		writeSentinelPnpmBuildAllowance(repoDir)
	}
}

// writeSentinelPnpmBuildAllowance opts the fixture out of pnpm's own build-
// script gate, in pnpm-workspace.yaml rather than package.json's "pnpm" key:
// pnpm 10.34.5 was verified to ignore that key outright, printing "Ignored
// build scripts: sentinel-pkg" and leaving the negative control inert, while
// pnpm 10.33.0 honored it -- a silent fixture rot that only became visible
// once these proofs stopped skipping when the binary was missing.
//
// dangerouslyAllowAllBuilds, not onlyBuiltDependencies: the narrower key was
// verified to match a "file:" dependency only under its full resolved id
// (sentinel-pkg@file:vendor/sentinel-pkg), never its bare name, so an
// allowlist would silently go inert again the next time pnpm changes how it
// spells that id -- reinstating exactly the false green this fixture exists to
// prevent. The fixture has one dependency, so "all builds" is that one build,
// and disarming pnpm's separate gate is precisely what leaves --ignore-scripts
// as the only thing deciding whether the postinstall runs. This writes a
// throwaway temp directory, never a repository Coach analyzes.
func writeSentinelPnpmBuildAllowance(repoDir string) {
	Expect(os.WriteFile(filepath.Join(repoDir, "pnpm-workspace.yaml"), []byte("dangerouslyAllowAllBuilds: true\n"), 0o644)).To(Succeed())
}
