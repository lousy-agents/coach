...
var _ = Describe("coach codesignal --baseline --suggest-project-config", func() {
	...
	When("a candidate generated by --suggest-project-config --output is fed back into --project-config", func() {
		It("is accepted as schema-valid even though no Go backend is registered yet", func() {
			repo := newTempGitRepo()
			...
			Expect(string(stdout)).To(ContainSubstring(`"kind":"project_backend_unavailable"`), "the generated candidate must reach backend dispatch, proving --project-config accepts its schema")
