...
var _ = Describe("coach codesignal --baseline --check-project --project-language typescript (package-manager adapter detection)", func() {
	...
	When("a recognized npm lockfile is committed and the npm on PATH is an out-of-range major", func() {
	...
	When("a recognized npm lockfile carries no packageManager pin and a supported npm is on PATH", func() {
		It("reports checks.package_manager pass, classified by the probed npm rather than by an absent pin", func() {
			repo := newTempGitRepo()
			commitFile(repo, "package.json", `{"name":"example","version":"1.0.0"}`+"\n")
			commitFile(repo, "package-lock.json", `{"name":"example","lockfileVersion":3}`+"\n")
			...
			Expect(doc.Checks.PackageManager.Kind).To(Equal("npm"))
			Expect(doc.Checks.PackageManager.Version).To(Equal("11.4.1"), "the classified version is the one the manager on PATH reports")
		})
	})

	When("a recognized npm lockfile carries no packageManager pin and no npm is reachable on PATH", func() {
	...
	When("package.json pins npm@11 but the npm actually on PATH is 10", func() {
		It("reports fail/package_manager_version_unsupported with found_version from the probe, recording the pin without ever classifying by it", func() {
			repo := newTempGitRepo()
			commitFile(repo, "package.json", `{"name":"example","version":"1.0.0","packageManager":"npm@11.2.0"}`+"\n")
			commitFile(repo, "package-lock.json", `{"name":"example","lockfileVersion":3}`+"\n")

			path := pathWithStubNodeAndPackageManager("v24.9.9", "npm", "10.9.7")
			...
			Expect(doc.Checks.PackageManager.Code).To(Equal("package_manager_version_unsupported"))
			Expect(doc.Checks.PackageManager.Kind).To(Equal("npm"))
			Expect(doc.Checks.PackageManager.FoundVersion).To(Equal("10.9.7"), "an in-range pin must never stand in for the out-of-range binary that would actually run")
			Expect(doc.Checks.PackageManager.PinnedVersion).To(Equal("11.2.0"), "the pin is still recorded, so a customer can see what the manifest asked for")
		})
	})
	...
	When("the npm on PATH reports a prerelease build inside the numeric major range", func() {
		It("reports checks.package_manager fail/package_manager_version_unsupported, since the frozen range >=11 <12 excludes prereleases", func() {
			...
			path := pathWithStubNodeAndPackageManager("v24.9.9", "npm", "11.0.0-rc.1")
	...
	When("a recognized npm lockfile exists with a supported npm on PATH and no .npmrc at all", func() {
		It("reports checks.package_manager pass with no gap naming the npm adapter itself", func() {
			repo := newTempGitRepo()
			commitFile(repo, "package.json", `{"name":"example","version":"1.0.0"}`+"\n")
			commitFile(repo, "package-lock.json", `{"name":"example","lockfileVersion":3}`+"\n")
