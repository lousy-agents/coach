...
var _ = Describe("codesignalcli.RunConfirmedSetupAndRecheckReadiness (AC-SET-6)", func() {
	When("a confirmed project-package install succeeds and resolves the previously missing compiler", func() {
		It("reruns the complete readiness check in-process, replacing the stale pre-install compiler gap with a pass", func() {
			...
			before, err := codesignalcli.CheckProjectReadiness(repo, head, "project.json")
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(outcome.PostInstallReadiness.Checks.Compiler.State).To(Equal(codesignalcli.ReadinessPass), "the recheck must observe the just-installed compiler, not the stale pre-install snapshot")
			Expect(outcome.PostInstallReadiness.Checks.Compiler.Version).To(Equal(version))
		})
	})
