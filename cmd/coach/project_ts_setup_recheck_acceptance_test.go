...
var _ = Describe("codesignalcli.RunConfirmedSetupAndRecheckReadiness (AC-SET-6)", func() {
	...
	When("the confirmed install itself fails", func() {
		It("reports SetupOutcomeFailed without ever attempting a recheck, since RunConfirmedSetup returns a non-Succeeded outcome and RunConfirmedSetupAndRecheckReadiness never reaches CheckProjectReadiness", func() {
			repo := newTempGitRepo()
