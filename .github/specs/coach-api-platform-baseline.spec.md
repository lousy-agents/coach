...
- Operator-run (non-CI) verification: the `llm` profile completes a `repo_baseline_scan` end to end with at least one schema-valid `source=agent` judgment produced by llama.cpp. This, together with the CI stub smoke, is this spec's SGLang/AWS investment gate — a stub-only smoke validates plumbing, not real-model rubric behavior. The CLI PRD no longer owns that gate.
